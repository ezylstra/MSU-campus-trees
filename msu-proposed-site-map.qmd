...
Below is a simple interactive map with locations of trees observed on the MSU campus between 2017 and 2025. Proposed sites for Nature's Notebook are outlined in blue (with tentative names in the center). Trees that were observed in 2025 are shown as filled circles, and trees that were not observed in 2025 are shown with open circles. Hovering over a tree marker will display the tree species and accession number. Clicking on a circle will display that information, plus the years that tree was monitored and the total number of observations submitted. A summary of trees in each proposed site is below the map (sites that don't have any current trees are not listed).
...
trees %>%
  ...
  addLabelOnlyMarkers(data = sites_centroid, 
                      ...
                      lng = ~x,
                      label = ~lapply(
                        stringr::str_replace_all(stringr::str_wrap(sitename, width = 20), 
                                                 "\n", "<br>"),
                        htmltools::HTML
                      ),
